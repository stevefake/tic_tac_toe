...
class TicTacToe
  ...
  valid_responses = %w(A1 A2 A3 B1 B2 B3 C1 C2 C3)
    ...
    if board.values.include?(' ') == true
      ...
      if valid_responses.include?(response) == false
        ...
        if (board['A1'] == x1['A1']) && (board['A2'] == x1['A2']) && (board['A3'] == x1['A3'])
          ...
          loop do
            computer_move = valid_responses[rand(0..9)]
            # response = gets.chomp
            if valid_responses.include?(computer_move) == false
              p 'Not a valid entry, computer, try again.'
            elsif board[computer_move] == 'O'
              p 'Computer has already moved there and is picking a new location.'
            elsif board[computer_move] == 'X'
              p "#{player_x} has already moved there. Computer is picking a new location."
            else
